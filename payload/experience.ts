  ...
  list: [
    {
      ...
      descriptions: [
        ...
        '안정적인 재화 비즈니스 운영을 위해 Redis(Redisson) 분산 락 도입을 도입하여 동시성 이슈 해결, 결제/정산 시스템 데이터 무결성 확보 및 대규모 부하 테스트 검증',
        'Spring Scheduler 이중화 및 LLM API 트래픽 분산 전략으로 일 1만건 이상 메시지 안정적 생성/발송 시스템 구축',
        '핵심 기능의 사용자 경험(UX) 개선을 목표로 AWS SQS 기반 비동기 아키텍처 전환, WebClient 도입 등으로 채팅 응답 속도 수분→수초 이내로 대폭 개선',
    ...
    },
    {
      ...
      descriptions: [
        ...
        '다수 프로젝트의 장기적인 유지보수성 강화를 위해 JPA Entity 공통 모듈화 및 Nexus 배포로 코드 중복 제거, 데이터 모델 일관성·생산성 향상',
