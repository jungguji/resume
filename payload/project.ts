  ...
  list: [
    {
      ...
      descriptions: [
        {
          content: '기존 AI 톡 기능에 새로운 비즈니스 모델 추가를 위해, 유료화된 대화 기능을 위한 신규 재화 관리 시스템 설계 및 구현 '
        },
        {
          content: '이미 존재하던 인앱결제로 구매할 수 있는 "캐시"라는 재화를 통해 구매할 수 있도록 함'
        },
        {
          content: '재화의 구매, 소비, 만료, 소멸 등 재화에 대한 모든 거래를 정확히 기록하고, 데이터 무결성 및 재화 사용 내역에 대한 정확한 정산을 보장하기 위한 테이블 설계 및 구현'
        }
      ],
    },
    {
    ...
    },
    {
      ...
      descriptions: [
        {
          content: '제작 비용이 많이 드는 콘텐츠를 대신하여, 유저 유입과 리텐션을 위한 기능으로 AI 톡 기능을 개발. 크리에이터 IP를 활용하여 캐릭터의 메타 데이터를 기반으로 대화할 수 있는 기능을 구현.'
        },
        {
          content: '폴링 방식으로 구현하여, 유저가 채팅방을 생성하고 채팅을 입력하면 서버에서 해당 채팅을 LLM으로 전송하고 콜백을 받아 유저에게 답장을 보여주는 구조로 설계.'
        },
        {
          content: '초기에는 Queue 역할을 하는 RDB Table에 채팅 생성 시 상태를 "Q"로 저장하고, 1초 간격의 스케줄러가 상태가 "Q"인 데이터를 조회하여 LLM에 전송 후 상태를 "P"로 변경. 콜백이 오면 답장 내용을 업데이트하고 상태를 "S"로 변경하는 방식을 적용.'
        },
        {
          content: '사용량 증가로 인해 5분 내 답변이 생성되지 않는 채팅이 많아지는 문제가 발생함. 이는 상태가 "Q"인 데이터를 1개씩 순차적으로 LLM에 전송하면서 지연이 발생하여 5분을 초과하는 케이스가 증가한 것이 원인.'
        },
        {
          content: '이를 해결하기 위해 ThreadPoolTaskExecutor를 사용하여 서버의 코어 수에 맞게 스레드를 늘리고, CompletableFuture의 allOf()와 join()을 활용하여 비동기 처리 및 병렬 요청으로 처리 속도를 향상시킴.'
        },
        {
          content: '그러나 스레드 수 증가에도 불구하고 스케일 아웃의 한계와 블로킹 이슈가 지속되어, RestTemplate 대신 WebClient를 사용하여 더 많은 수의 요청을 비동기로 발송하도록 개선.'
        },
        {
          content: '최종적으로 LLM과의 통신에 메시지 큐를 도입하여, 채팅 서버에서는 답장 생성 요청을 메시지 큐에 담기만 하고, LLM에서도 콜백대신 답장을 메시지 큐에 담고, 큐를 통해 가져와 일괄 업데이트하는 방식으로 구조를 개선. 이를 통해 스케일 아웃 문제를 해결하고 응답 지연을 최소화함.'
        },
      ],
    },
    {
