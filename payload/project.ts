  ...
  list: [
    {
      ...
      descriptions: [
        {
          content:
            '19만 사용자에게 전송하는데 8시간 정도 걸리던 푸시를 30분 안에 발송할 수 있도록 개선',
        },
        {
          content:
            'Spring의 @Schedubled를 이용해 1시간마다 등록된 스케줄을 카카오 푸시 API와 FCM, APNs를 활용해 광고 푸시를 전송하고 있었는데, ' +
            '전체 사용자에게 발송 시 8시간 정도 시간이 소요 되어서, 하루에 1번 이상의 푸시를 발송할 수가 없어서 광고나 이벤트 진행에 문제가 존재하였음',
        },
        {
          content:
            '이를 해결하기 위해, 푸시 서비스를 위한 스레드 풀을 생성하여, 푸시 하나당 스레드 하나씩을 할당하여 동시에 여러 개의 푸시를 전송할 수 있게 수정하였고, ' +
            '이렇게 개선하여 8시간이 소요되던 푸시를 30분으로 개선하여, 하루에 여러 건의 광고, 이벤트 푸시를 발송할 수 있게 함',
        },
      ],
    },
    {
      ...
      descriptions: [
        ...
        { 
            content: '이를 해결하는 방법으로 Entity class들을 따로 모은 프로젝트를 별도로 구성하여 내부 Maven 저장소(Nexus)에 Jar형태로 업로드하고, 다른 프로젝트들에서 이를 추가하여 사용하도록 개선'
        },
      ],
    },
    {
      ...
      descriptions: [
        {
          content:
            '페이스북 광고에 연결한 이벤트 페이지에 사용자가 도달하는 광고 노출 수 대비 비정상적으로 적게 노출되는 이슈가 발생',
        },
        {
          content:
            '광고에 노출한 이벤트 페이지의 로딩이 너무 느려서 발생하는 문제로 판단하고 확인 결과, JSP + jQuery로 구성된 해당 이벤트 페이지 로딩이 평균 9초 정도 걸리는 것을 확인하였음'
        },
        {
          content: 
            '로딩 속도 개선을 위해 페이지 조회 시 보이는 상단 부분만 먼저 로딩될 수 있도록 상단 렌더링 시 필요한 데이터만 Jsp와 ModelAttribute에 먼저 보내고 ' +
            '페이지 하단 렌더링 시 필요한 데이터들은 Jsp에서 ajax를 활용하여 비동기로 호출하도록 개선하여서, 평균 로딩 시간을 9초에서 2초 대로 개선'
        },
      ],
    },
    {
